...
public class FrictionTest
{
    ...
    public IEnumerator DynamicFrictionCoefficientIsUsed()
    {
        ...
        Assert.AreEqual(startHeight, roughCube.transform.position.y, 1e-4f, "Rough cube should be on the ground");
        Assert.AreEqual(startHeight, smoothCube.transform.position.y, 1e-4f, "Smooth cube should be on the ground");
        Assert.AreEqual(0, roughCube.transform.rotation.eulerAngles.x, 0.1f, "Rough cube should be facing forwards");
        Assert.AreEqual(0, smoothCube.transform.rotation.eulerAngles.x, 0.1f, "smooth cube should be facing forwards");

        Vector3 force = Vector3.forward * 5;
        ...
        Assert.AreEqual(startHeight, roughCube.transform.position.y, 1e-4f, "Rough cube should stay on the ground");
        Assert.AreEqual(startHeight, smoothCube.transform.position.y, 1e-4f, "Smooth cube should stay on the ground");
        Assert.AreEqual(0, roughCube.transform.rotation.eulerAngles.x, 0.1f, "Rough cube should not roll");
        Assert.AreEqual(0, smoothCube.transform.rotation.eulerAngles.x, 0.1f, "Smooth cube should not roll");
    }
}
